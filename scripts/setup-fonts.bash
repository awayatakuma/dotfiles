...
FONT_NAME="UDEVGothic"
VARIANT="JPDOC"
FONT_DIR="$XDG_DATA_HOME/fonts/${FONT_NAME}${VARIANT}"
...
echo "Installing ${FONT_NAME} ${VARIANT} v$VERSION"
...
if [ -d "$FONT_DIR" ]; then
    echo "${FONT_NAME}${VARIANT} already exists."
    exit 0
fi
...
# Copy ONLY JPDOC variant .ttf files from the extracted folder
cp "$TMP_DIR/${FONT_NAME}_v${VERSION}"/*${VARIANT}*.ttf "$FONT_DIR/"
